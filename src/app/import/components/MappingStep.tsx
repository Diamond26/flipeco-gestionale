    ...
    <div className="flex flex-col gap-6 animate-fade-in w-full max-w-4xl mx-auto pt-4">
      ...
      <div className="relative bg-white/[0.03] backdrop-blur-2xl rounded-3xl border border-white/10 p-7 lg:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.2)] overflow-hidden group">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-2">
          <h3 className="text-[17px] font-bold text-white tracking-wide">Mappatura Colonne</h3>
          <span className="text-[13px] text-white/40">File: <span className="text-white/70">{selectedFile?.name}</span></span>
        </div>
        <p className="text-[13px] text-white/50 mb-8 leading-relaxed max-w-3xl">
            ...
            return (
              <div key={key} className="flex flex-col gap-1.5">
                <label className="text-[13px] font-semibold tracking-wide flex items-center gap-1">
                  <span className={required && isError ? 'text-red-400' : 'text-white/80'}>{label}</span>
                ...
                <div className="relative z-10 w-full group/sel">
                  <select
                    className={`
                      w-full appearance-none bg-white/[0.02] border rounded-xl px-4 py-2.5 text-[14px] transition-all cursor-pointer shadow-inner backdrop-blur-sm
                      focus:outline-none
                      ${isError 
                        ? 'border-red-500/50 text-white shadow-[0_0_15px_rgba(239,68,68,0.15)] ring-1 ring-red-500/20' 
                        : isSuccess 
                          ? 'border-[#7BB35F]/50 text-white shadow-[0_0_15px_rgba(123,179,95,0.15)]' 
                  ...
                  >
                    <option value="" className="bg-[#0b0f19] text-white/50">— non mappare —</option>
                    {parsedHeaders.map(h => (
                      <option key={h} value={h} className="bg-[#0b0f19] text-white">{h}</option>
                    ))}
                  </select>
      ...
      {/* Preview */}
      {parsedRows.length > 0 && (
        <div className="relative bg-white/[0.03] backdrop-blur-2xl rounded-3xl border border-white/10 p-7 lg:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.2)]">
          <h3 className="text-[16px] font-bold text-white mb-5 tracking-wide">Anteprima Dati (prime 3 righe)</h3>
              ...
              <thead>
                <tr className="bg-white/5">
                  {parsedHeaders.map((h, i) => (
                    <th key={i} className="px-4 py-3.5 text-left text-[11px] font-bold uppercase tracking-wider text-white/40 whitespace-nowrap border-b border-white/5">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {parsedRows.slice(0, 3).map((row, idx) => (
                  <tr key={idx} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors">
                    {parsedHeaders.map((h, i) => (
                      <td key={i} className="px-4 py-3 text-[13px] text-white/70 whitespace-nowrap">
                        {row[h] ?? ''}
                      </td>
                    ))}
                  </tr>
