...
export function POSSearch({
  ...
  return (
    <div className="w-full h-full flex flex-col gap-4 relative z-10">
      <div className="bg-card rounded-3xl border border-black/[0.04] dark:border-white/[0.06] p-6 shadow-[0_1px_3px_rgba(0,0,0,0.08)] dark:shadow-[0_1px_3px_rgba(0,0,0,0.3)] relative overflow-hidden flex flex-col gap-5">
        ...
        <form onSubmit={handleBarcodeScan} className="relative z-10 flex flex-col gap-3">
          <div ref={scannerBoxRef} className="relative w-full">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-4 text-xl font-mono text-foreground border border-black/[0.08] dark:border-white/[0.08] rounded-2xl bg-surface-light/50 dark:bg-white/[0.03] focus:outline-none focus:ring-4 focus:ring-brand/15 focus:border-brand/40 transition-all placeholder:text-foreground/30 placeholder:font-sans"
              autoComplete="off"
            />
            {scannerOpen && scannerSuggestions.length > 0 && (
              <div className="absolute left-0 right-0 top-[calc(100%+8px)] z-50 rounded-2xl border border-black/[0.08] dark:border-white/[0.08] bg-card shadow-xl overflow-hidden">
                    ...
                    return (
                      <li key={product.id}>
                        <button
                          ...
                          className={cn(
                            'w-full text-left px-5 py-3 border-b last:border-b-0 border-black/[0.04] dark:border-white/[0.06] transition-colors',
                        ...
                        >
                          <div className="flex items-center justify-between gap-3">
                            ...
                            <div className="text-right shrink-0">
                              <p className="text-[15px] font-bold text-brand">{formatCurrency(product.sell_price)}</p>
          ...
          <Button
            ...
            size="lg"
            loading={scanLoading}
            className="w-full py-4 text-lg rounded-2xl h-auto"
          >
        ...
        <button
          ...
          className="w-full rounded-2xl py-4 flex items-center justify-center gap-2 font-bold text-[15px] tracking-wide border-2 border-black/[0.06] dark:border-white/[0.08] text-foreground hover:bg-surface-light/60 dark:hover:bg-white/[0.04] transition-colors group bg-surface-light/30 dark:bg-white/[0.02]"
        >
