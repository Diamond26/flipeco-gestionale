...
export function POSSearch({
  ...
  return (
    <div className="w-full h-full flex flex-col gap-4 relative z-10">
      <div className="bg-white/90 dark:bg-white/[0.02] backdrop-blur-2xl rounded-3xl border border-surface/50 dark:border-white/[0.06] p-6 shadow-2xl dark:shadow-[0_20px_50px_rgba(0,0,0,0.3)] relative overflow-hidden flex flex-col gap-6 transition-all duration-500">
        ...
        <div className="relative z-10">
           ...
           <p className="text-xs text-foreground/70">Digita il nome o scansiona direttamente il codice a barre.</p>
        </div>
        ...
        <form onSubmit={handleBarcodeScan} className="relative z-10 flex flex-col gap-3">
          <div ref={scannerBoxRef} className="relative w-full group">
            ...
            <input
              ...
              className="w-full pl-14 pr-4 py-5 text-2xl font-mono text-foreground border border-surface/60 dark:border-white/[0.08] rounded-2xl bg-surface-light/80 dark:bg-black/40 focus:outline-none focus:ring-4 focus:ring-brand/15 focus:border-brand/40 shadow-inner transition-all placeholder:text-foreground/40 placeholder:font-sans placeholder:text-lg"
              autoComplete="off"
            />
            {scannerOpen && scannerSuggestions.length > 0 && (
              <div className="absolute left-0 right-0 top-[calc(100%+12px)] z-50 rounded-2xl border border-surface/50 dark:border-white/[0.08] bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                <ul className="max-h-80 overflow-y-auto py-2 divide-y divide-surface/40 dark:divide-white/[0.06]">
                        ...
                        >
                          <div className="flex items-center justify-between gap-3">
                            <div className="min-w-0">
                              ...
                              <p className="text-[11px] text-foreground/60 truncate mt-0.5 uppercase tracking-wider">
                            ...
                            <div className="text-right shrink-0">
                              ...
                              <p className="text-[10px] uppercase font-semibold text-foreground/50">{product.quantity} disp.</p>
                            </div>
                          </div>
                        </button>
                      </li>
        ...
        <div className="relative py-2">
           <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-surface/40 dark:border-white/[0.08]" /></div>
           <div className="relative flex justify-center"><span className="bg-white dark:bg-[#1a1f2e] px-4 text-[10px] font-bold text-foreground/60 tracking-[0.2em]">OPPURE</span></div>
        </div>

        <button
          type="button"
          onClick={() => setProductMenuOpen(true)}
          className="w-full rounded-2xl py-4 flex items-center justify-center gap-3 font-bold text-[14px] uppercase tracking-widest border-2 border-surface/50 dark:border-white/[0.08] text-foreground/80 hover:text-foreground hover:bg-surface-light/60 dark:hover:bg-white/[0.04] transition-all group bg-surface-light/30 dark:bg-white/[0.01] overflow-hidden relative shadow-sm"
        >
